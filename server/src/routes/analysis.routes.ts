  ...
  app.get('/analysis/:id/report.pdf', { preHandler: [app.authenticate] }, async (req, reply) => {
    ...
    const FRAMES = {
      ru: {
        ...
        aboutText: 'Сформировано Lexab. Отчёт подготовлен с помощью ИИ и не является юридической консультацией.',
        title: 'Lexab — Отчёт о проверке договора',
      },
      uz: {
      ...
      en: {
        ...
        aboutText: 'Generated by Lexab contract intelligence. This report is an AI-assisted review and does not constitute legal advice.',
        title: 'Lexab — Contract Review Report',
      },
    } as const;
    ...
    reply.header('Content-Disposition', attachmentDisposition(`Lexab_Report_${a.fileName}`, 'pdf'));
