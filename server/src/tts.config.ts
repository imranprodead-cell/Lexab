...
/** 20 с, не 10: живой замер 2026-07-27 — здоровый Gemini 3.1/2.5 на куске в
 *  ~390 символов работает 11-14 с (скорость ≈30 симв/с); при 10-15 с попытка
 *  обрывалась на полуслове и весь ролик уезжал в Chirp. Куски укорочены до
 *  ~300 симв. (см. TTS_CHUNK_TARGET_CHARS) — синтез ~10 с с запасом. */
export const TTS_ATTEMPT_TIMEOUT_MS = 20_000;
...
/** Баланс: крупнее куски → цельнее интонация, но дольше синтез одного куска.
 *  Живой замер: Gemini ≈30 симв/с → 300 симв. ≈ 10 с синтеза (в 20-секундный
 *  таймаут с запасом) и ≈20 с звука — префетч успевает готовить следующий
 *  кусок, пока играет текущий. 700 симв. требовали 20-25 с и обрывались. */
