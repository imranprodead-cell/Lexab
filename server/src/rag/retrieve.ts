...
const REWRITE_SYSTEM: Record<string, string> = {
  ...
  UZ: 'Перепиши вопрос пользователя как 2-3 коротких поисковых запроса по русскоязычным текстам законов Узбекистана: используй термины законодателя («неустойка», «расторжение договора», «ненадлежащее исполнение обязательства»). Вопрос может быть задан на узбекском или английском — запросы всё равно пиши ТОЛЬКО по-русски, переводя понятия в русские термины законодателя (neustoyka/penya → «неустойка», shartnomani bekor qilish → «расторжение договора», elektron imzo → «электронная цифровая подпись»). Запрос вида «ст. 260 ГК» включай ТОЛЬКО если абсолютно уверен в номере статьи; никогда не угадывай номер — неверный номер хуже, чем запрос без номера. Корпус включает и указы/постановления Президента (УП/ПП): запрос вида «п. 5 УП-6079» включай ТОЛЬКО если пользователь явно спрашивает про конкретный указ или постановление — иначе ищи по терминам, не по номерам актов. Только по-русски. Без преамбулы.',
...
const RU_ALIASES: Record<string, RuAlias[]> = {
  UZ: [
    // ГПК СТРОГО ВЫШЕ общего /Гражданск/ («первое совпадение выигрывает»):
    // иначе «ст. 45 Гражданского процессуального кодекса» уехала бы в ГК.
    { re: /Гражданск\S*\s+процессуальн/i, pattern: '%Гражданский процессуальный кодекс%' },
    { re: cyrWord('ГПК'), pattern: '%Гражданский процессуальный кодекс%' },
    ...
    // ── Волна 3 (2026-07-28): договорное ядро — таможня, страхование, ИС,
    //    реклама, техрегламент, транспорт, персданные, э-коммерция. Ключевые
    //    слова специфичны (коллизий с существующими алиасами нет). ──────────
    { re: /таможенн/i, pattern: 'Таможенный кодекс%' },
    { re: /страхов/i, pattern: '%О страховой деятельности%' },
    { re: /авторск/i, pattern: '%Об авторском праве%' },
    { re: /реклам/i, pattern: '%О рекламе%' },
    { re: /техническ\S*\s+регулирован/i, pattern: '%О техническом регулировании%' },
    { re: /транспорт/i, pattern: '%О транспорте%' },
    { re: /изобретени/i, pattern: '%Об изобретениях%' },
    { re: /товарн\S*\s+знак|знак\S*\s+обслуживани/i, pattern: '%О товарных знаках%' },
    { re: /персональн\S*\s+данн/i, pattern: '%О персональных данных%' },
    { re: /электронн\S*\s+коммерци/i, pattern: '%Об электронной коммерции%' },
    // ── Волна 4 (2026-07-28): исполнение, нотариат, стройка, банки/платежи,
    //    ценные бумаги, третейские/МКА, ГЧП, бухучёт. «исполнени» без «судебн»
    //    не берём — слишком общее слово для договоров. ─────────────────────
    { re: /исполнени\S*\s+судебн/i, pattern: '%Об исполнении судебных актов%' },
    { re: /нотариат|нотариальн/i, pattern: '%О нотариате%' },
    { re: /градостроительн/i, pattern: 'Градостроительный кодекс%' },
    { re: /платеж\S*\s+систем|о\s+платежах/i, pattern: '%О платежах и платежных системах%' },
    { re: /ценн\S*\s+бумаг/i, pattern: '%О рынке ценных бумаг%' },
    { re: /третейск/i, pattern: '%О третейских судах%' },
    { re: /международн\S*\s+коммерческ\S*\s+арбитраж/i, pattern: '%О международном коммерческом арбитраже%' },
    { re: /банк\S*\s+деятельност|о\s+банках/i, pattern: '%О банках и банковской деятельности%' },
    { re: /государственно-частн/i, pattern: '%О государственно-частном партнерстве%' },
    { re: /бухгалтерск/i, pattern: '%О бухгалтерском учете%' },
    // ── Волна 5 (2026-07-28): жильё, энергетика, аудит, гарантии бизнеса,
    //    микрофинансы, лизинг, ВЭД. «Жилищн» ДО общего «Градостроительн» не
    //    конфликтует (разные слова). ────────────────────────────────────────
    { re: /жилищн/i, pattern: 'Жилищный кодекс%' },
    { re: /электроэнергетик/i, pattern: '%Об электроэнергетике%' },
    { re: /аудиторск/i, pattern: '%Об аудиторской деятельности%' },
    { re: /гаранти\S*\s+свобод\S*\s+предпринимательск/i, pattern: '%О гарантиях свободы предпринимательской%' },
    { re: /небанковск\S*\s+кредитн|микрофинанс/i, pattern: '%О небанковских кредитных организациях%' },
    { re: /лизинг/i, pattern: '%О лизинге%' },
    { re: /внешнеэкономическ/i, pattern: '%О внешнеэкономической деятельности%' },
    // ── Волна 6 (2026-07-29): коммерческая тайна, э-документооборот, оценка,
    //    биржи, СЭЗ, экспортный контроль, защита собственности, админ-
    //    процедуры, медиация. ГПК-алиасы — в самом верху списка (порядок!). ──
    { re: /коммерческ\S*\s+тайн/i, pattern: '%О коммерческой тайне%' },
    { re: /электронн\S*\s+документооборот/i, pattern: '%Об электронном документообороте%' },
    { re: /оценочн/i, pattern: '%Об оценочной деятельности%' },
    { re: /бирж/i, pattern: '%О биржах и биржевой деятельности%' },
    { re: /специальн\S*\s+экономическ\S*\s+зон/i, pattern: '%О специальных экономических зонах%' },
    { re: cyrWord('СЭЗ'), pattern: '%О специальных экономических зонах%' },
    { re: /экспортн\S*\s+контрол/i, pattern: '%Об экспортном контроле%' },
    { re: /защит\S*\s+частн\S*\s+собственност/i, pattern: '%О защите частной собственности%' },
    { re: /административн\S*\s+процедур/i, pattern: '%Об административных процедурах%' },
    { re: /медиаци/i, pattern: '%О медиации%' },
    ...
    // «Фуқаролик Кодексининг 260-моддаси» (живой кейс). «процессуал» —
    // ВЫШЕ «кодекс»-формы (ГПК с волны 6 в корпусе); «кодекс» сразу после
    // названия обязателен, чтобы ГК не хватал чужие кодексы.
    { re: /фу[қк]аролик\s+процессуал/i, pattern: '%Гражданский процессуальный кодекс%' },
